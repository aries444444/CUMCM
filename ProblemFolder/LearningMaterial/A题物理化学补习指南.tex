...
\begin{abstract}
本指南面向数学背景、物理化学零基础的建模者，目标是用 3--5 天补到能独立写出并数值求解本题所需的全部方程。第一节为题目识别，第二至七节按关键词讲解物理化学知识，第八节给出四大问题的完整方程组装，第九、十节为验证清单与补习路线图，第十一节汇总易错点。
\end{abstract}
...
\textbf{本质}：一根长圆柱药材（长 25\,cm、半径 2\,cm）放在烘房里用热风吹干。烘房温度、湿度随时间变化。要预测药材\textbf{内部每一点的温度 $T(r,t)$ 和水分浓度 $C(r,t)$}（干基含水率，kg 水/kg 干料）。
...
\item 附件 2 的半径数据覆盖完整 72\,h，问题 4 直接用数据插值 $R(t)$（三次样条平滑后再求 $R'$）。
\end{enumerate}
...
固体表面与流体的换热速率正比于温差：
\begin{equation}
q_{\mathrm{conv}}=h\,(T_{\mathrm{air}}-T_{\mathrm{surface}}) \qquad (h\text{ 为对流换热系数，W/(m}^2\cdot\text{K)})
\end{equation}
...
\begin{equation}
\mathrm{Bi}=\frac{\text{内部导热阻力}}{\text{表面对流阻力}}=\frac{hL_c}{k},\qquad L_c=\frac{V}{A}=\frac{R}{2}\ \text{（长圆柱特征长度）}
\end{equation}
\begin{itemize}
...
题给 $D=7\times10^{-9}\,e^{-0.89C}$（问题 1）或 $D=2.4\times10^{-3}\,e^{-0.45C}\,e^{-3850/T}$（问题 2--4）：$D$ 是 $C$（和 $T$）的函数 $\Rightarrow$ 方程是\textbf{拟线性抛物方程}，必须数值求解；解析解只对「冻结 $D$」成立（可作验证）。

\subsection{对流传质毕渥数 $\mathrm{Bi}_m=h_m R/D$}
问题 1：$\mathrm{Bi}_m=8\times10^{-7}\times0.02\,/\,7\times10^{-10}\approx22\gg1$ $\Rightarrow$ 内部扩散阻力主导，表面 $C$ 很快趋近 $C_{\mathrm{air}}$（表面边界近似 Dirichlet）。这是很好的论文分析点。
...
\item \textbf{水分比} $\mathrm{MR}=(C-C_{eq})/(C_0-C_{eq})$：文献常用无量纲量，画干燥曲线、验证模型时用。
\end{itemize}

\subsection{本题的模型如何自动体现三阶段}
扩散$+$对流传质模型本身就会给出「表面先干、中心滞后」的行为。论文中结合模拟曲线讨论「预热段失水仅限表面约 1\,mm 薄层（$\sqrt{Dt}$ 估算）、恒温段速率趋稳、降速段指数衰减」会非常加分。
...
温度对过程速率的指数放大：
\begin{equation}
k_{\mathrm{rate}}=A\,e^{-E_a/(R_g T)}
\end{equation}
$E_a$ 为活化能（J/mol），$R_g=8.314$\,J/(mol$\cdot$K)，\textbf{$T$ 必须用开尔文}。本题 $D$ 里的 $e^{-3850/T}$：对比知 $E_a/R_g=3850\Rightarrow E_a\approx32.0$\,kJ/mol。食品/药材干燥实测活化能典型 12--45\,kJ/mol，题给公式在合理区间内。
...
\item 表面蒸发会\textbf{带走热量}（蒸发冷却）：$q_{\mathrm{evap}}=L_v\,J_{\mathrm{evap}}$。
...
\subsection{时间推进与稳定性}
\begin{itemize}
\item 显式 Euler：$\Delta t\le\Delta r^2/(2\alpha_{\max})$（圆柱中心更严）。本题 $\alpha\approx1.7\times10^{-7}$，$\Delta r=1$\,mm 时 $\Delta t\le3$\,s；$\Delta t=1$\,s 勉强可行但太贴边。
...
令 $\zeta=r/R(t)\in[0,1]$，把时变域 $[0,R(t)]$ 变成固定域 $[0,1]$：
\begin{align}
...
量 & 符号 & 单位 \\
\midrule
温度 & $T$ & $^\circ$C（输出）/ K（代入 $e^{-3850/T}$） \\
水分浓度 & $C$ & kg/kg（无量纲） \\
密度 & $\rho$ & kg/m$^3$（题给） \\
比热容 & $c_p$ & J/(kg$\cdot$K)（题给） \\
热传导系数 & $k$ & W/(m$\cdot$K)（题给） \\
对流换热系数 & $h$ & W/(m$^2\cdot$K)，$=25$ \\
对流传质系数 & $h_m$ & m/s，$=8\times10^{-7}$ \\
扩散系数 & $D$ & m$^2$/s（公式算） \\
\bottomrule
\end{tabular}
\end{center}

自查：$\rho c_p\,\partial T/\partial t$ 与 $k\nabla^2T$ 同为 W/m$^3$；$\partial C/\partial t$ 与 $D\nabla^2C$ 同为 s$^{-1}$；$-D\,\partial C/\partial r$ 与 $h_m(C-C_{\mathrm{air}})$ 同为 (kg/kg)$\cdot$m/s。\textbf{$r$ 全程用米}（$R=0.02$\,m），时间用秒。
...
\end{equation}
\begin{equation}
\text{质：}\quad \frac{\partial C}{\partial t}=\frac{1}{r}\frac{\partial}{\partial r}\Big(r\,D(C)\,\frac{\partial C}{\partial r}\Big),\qquad D=7\times10^{-9}\,e^{-0.89C}
\end{equation}
\begin{align*}
...
\item 预期形态：1800\,s 内温度从外向内渗透（$\sqrt{\alpha t}\approx1.7$\,cm），水分只在表面约 1\,mm 薄层变化（$\sqrt{Dt}\approx1$\,mm），中心 $C$ 几乎不动。
\item \textbf{附件 1 实际数据}：前 1800\,s 烘房仅升至 $\approx29.7\,^\circ$C、$C_{\mathrm{air}}\approx0.021$ —— 预热初期驱动很弱，药材温度、水分变化都很小（可作为论文 sanity check：表 1 数值应非常接近初值）。
...
\end{equation}
\begin{equation}
D=2.4\times10^{-3}\,e^{-0.45C}\,e^{-3850/T}\quad \leftarrow\ T\ \text{用开尔文！}
\end{equation}
...
\item $t^*$ 由模拟给出；为精确到「秒」可在达标时刻附近加密或插值。
\item 预期量级（供标定直觉）：降速段占主导，时间 $\sim R^2/\bar{D}$ 量级。$\bar{D}\sim10^{-9}$--$10^{-8}$\,m$^2$/s、$R^2=4\times10^{-4}$ $\Rightarrow$ 数十小时量级。注意恒温段 \textbf{$C_{\mathrm{air}}\approx0.05\neq0$}，目标 0.15 高于 $C_{\mathrm{air}}$，可达到但尾段会减速；表 5 按每 6\,h 出表、附件 2 半径数据到 72\,h，暗示 $t^*$ 大概率在 10--70\,h 之间。\textbf{答案以模拟为准，不必强行凑「2--3 天」}。
...
\item \textbf{守恒检验}：总水量减少量 $=$ 表面累计通量 $\int h_m(C_s-C_{\mathrm{air}})\,dA\,dt$（数值算两边对比）。
...
\item \textbf{Bi 数分析}：$\mathrm{Bi}\approx0.7$ $\Rightarrow$ 内部温度梯度不可忽略（呼应用 PDE 的必要性）；$\mathrm{Bi}_m\approx22$ $\Rightarrow$ 表面水分快速跟随烘房。
\end{enumerate}
