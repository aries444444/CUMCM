...
# ---------- 参数（附录2） ----------
R, dr, N = 0.02, 3.125e-5, 641     # 半径 2 cm，主网格空间步长 0.03125 mm，641 个节点
...
if __name__ == "__main__":
    ...
    # 表2 表面处存在陡峭的浓度边界层，0.125 mm 网格下 (100s,表面) 误差约 4.7e-4，
    # 故主网格取 0.03125 mm（误差约 3e-5，四位小数可靠）；result1.xlsx 仍按模板输出每 1 s、每 0.1 cm。
    ...
    xcols = list(range(0, N, 32))     # 0:0.1:2 cm（0.03125 mm 网格上每 32 节点一列）
    for ws in (wsT, wsC):
    ...
    # ---------- 验证1：Bessel 解析锚点（常数边界 T_air=41.513，主网格 Δr=0.03125 mm） ----------
    Ta = 41.513
    Bi = h * R / k
    ...
    # ---------- 验证2：网格收敛（0.125 mm 与 0.0625 mm 两套粗网格，Δt=0.125 s 固定，与主网格同位置采样） ----------
    global_dr = dr; global_N = N
    dr = 1.25e-4; N = 161
    ri2 = [0, 40, 80, 120, 160]            # 0.125 mm 网格上的 0 / 0.5 / 1 / 1.5 / 2 cm
    T2, C2, T2f, C2f = solve(14400, 0.125, T_air, C_air, record={tt: ri2 for tt in t_ev})
    dr = 6.25e-5; N = 321
    ri1 = [0, 80, 160, 240, 320]           # 0.0625 mm 网格上的同一物理位置
    ...
    # ---------- 验证3：离散质量守恒（0.125 mm 与 0.03125 mm 两套网格上分别做） ----------
    ...
    lhs3, rhs3 = check_conserve(C, 3.125e-5, 641, 0.125, 14400)   # 主网格
    print('验证3 离散守恒: 0.125mm 网格 ΣvΔC=%.6e vs -R∫h_m(C_R-C_air)dt=%.6e，差 %.2e'
          % (lhs2, rhs2, lhs2 - rhs2))
    print('              0.03125mm 网格 ΣvΔC=%.6e vs -R∫h_m(C_R-C_air)dt=%.6e，差 %.2e'
          % (lhs3, rhs3, lhs3 - rhs3))
