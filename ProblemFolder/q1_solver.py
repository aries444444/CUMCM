...
圆柱轴对称热传导 + 水分扩散（非线性 D(C)），守恒型有限差分 + Crank--Nicolson + Thomas。
输出：表1/表2（控制台）、result1.xlsx（两个 sheet，1800 行 x 21 列，保留 4 位小数）、
四重验证（Bessel 解析锚点 / 网格收敛 / 质量守恒 / 渐近行为）。
"""
import sys, os
import numpy as np
import openpyxl
...
# ---------- 参数（附录2） ----------
R, dr, N = 0.02, 1.25e-4, 161      # 半径 2 cm，主网格空间步长 0.125 mm，161 个节点
...
# ---------- 主计算：附件1 时变边界，0--1800 s，Δr=0.125 mm，Δt=0.125 s ----------
# 表2 表面处存在陡峭的浓度边界层，0.25 mm 网格下表面浓度误差约 2e-3，
# 故主网格取 0.125 mm（误差约 5e-4，四位小数可信）；result1.xlsx 仍按模板输出每 1 s、每 0.1 cm。
t_ev = [100, 300, 600, 900, 1200, 1500, 1800]
ri = [0, 40, 80, 120, 160]        # 0, 0.5, 1, 1.5, 2 cm
...
xcols = list(range(0, N, 8))      # 0:0.1:2 cm（0.125 mm 网格上每 8 节点一列）
for ws in (wsT, wsC):
...
# ---------- 验证1：Bessel 解析锚点（常数边界 T_air=41.513，主网格 Δr=0.125 mm） ----------
Ta = 41.513
Bi = h * R / k
...
# ---------- 验证2：网格收敛（0.25 mm 与 0.5 mm 两套粗网格，与主网格同位置采样） ----------
global_dr = dr; global_N = N
dr = 2.5e-4; N = 81
ri2 = [0, 20, 40, 60, 80]              # 0.25 mm 网格上的 0 / 0.5 / 1 / 1.5 / 2 cm
T2, C2, T2f, C2f = solve(7200, 0.25, T_air, C_air, record={tt: ri2 for tt in t_ev})
dr = 5e-4; N = 41
ri1 = [0, 10, 20, 30, 40]              # 0.5 mm 网格上的同一物理位置
T1, C1, T1f, C1f = solve(3600, 0.5, T_air, C_air, record={tt: ri1 for tt in t_ev})
dr = global_dr; N = global_N
print('验证2 收敛: 表1 最大差 0.25mm->0.125mm: %.2e °C, 0.5mm->0.25mm: %.2e °C'
      % (np.max(np.abs(T2 - Ttab)), np.max(np.abs(T1 - T2))))
print('         表2 最大差 0.25mm->0.125mm: %.2e, 0.5mm->0.25mm: %.2e kg/kg'
      % (np.max(np.abs(C2 - Ctab)), np.max(np.abs(C1 - C2))))

# ---------- 验证3：离散质量守恒（0.125 mm 与 0.25 mm 两套网格上分别做） ----------
...
lhs2, rhs2 = check_conserve(C2f, 2.5e-4, 81, 0.25, 7200)
lhs3, rhs3 = check_conserve(C, 1.25e-4, 161, 0.125, 14400)   # 主网格
print('验证3 离散守恒: 0.25mm 网格 ΣvΔC=%.6e vs -R∫h_m(C_R-C_air)dt=%.6e，差 %.2e'
      % (lhs2, rhs2, lhs2 - rhs2))
print('              0.125mm 网格 ΣvΔC=%.6e vs -R∫h_m(C_R-C_air)dt=%.6e，差 %.2e'
      % (lhs3, rhs3, lhs3 - rhs3))

# ---------- 验证4：渐近行为（常数边界，长时程，1 mm 网格） ----------
Ta, Ca = 41.513, 0.04986
ri1mm = [0, 5, 10, 15, 20]            # 1 mm 网格上的 0 / 0.5 / 1 / 1.5 / 2 cm
