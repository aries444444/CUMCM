...
药材为长 25 cm、半径 2 cm 的细长圆柱，长径比达 6.25。轴向与径向的扩散特征时间之比约为 $(L/(2R))^2\approx39$（轴向特征尺度取半长 $L/2$，即到最近端面的距离），故端面效应与轴向不均匀性可以忽略，药材内部的温度与水分梯度主要由径向热风对流引起。将药材视为无限长圆柱，取径向一维轴对称模型即可（文献中圆柱形药材、食品的干燥均广泛采用径向一维模型，如人参、豆类、胡萝卜等\cite{simal,abbasi,kim}）。周向按轴对称处理，初始条件均匀，故解不依赖周向角。
...
以下结果均按题面要求保留四位小数，并存入 result1--4.xlsx。其中 result1/2.xlsx 含「温度」「水分浓度」两个工作表，A 列为时间（s），第 1 行为到药材中心的距离（cm）；result3/4.xlsx 单表存水分浓度，其末行为烘干结束时刻 $t^*$（205701 s / 182897 s）的一行，与表 5、表 6 的末行一致。
...
结果分析：干燥过程呈明显的两阶段形态。前 12 h 含水率快速下降（表面由 2.55 降至 0.164 kg/kg），此阶段为表面传质主导的快速干燥段（干燥速率由表面向环境的对流传质通量控制）；此后干燥速率显著放缓，进入降速干燥段——水分由内部向表面扩散的路径越来越长，且 $D$ 随 $C$ 下降按 $\mathrm{e}^{-0.45/C}$ 急剧衰减，表面含水率 54 h 时仅 0.0528 kg/kg，已逼近烘房水分浓度 0.04986 kg/kg（表面传质驱动力趋于消失）。
...
结果分析：半径在干燥前期快速收缩（前 3.5 h 内由 2 cm 缩至约 1.5 cm），故 1.5 cm、2 cm 两列自 6 h 起即已落在药材之外。与问题 3 对比：$t^*$ 由 57.14 h 缩短为 50.80 h，缩短约 6.3 h（11.1\%）。这一净效应由两个相反方向的因素合成：附录 4 的扩散系数比附录 3 小约 2--5 倍（高含水率端约 5 倍、低含水率端约 2 倍），使干燥变慢；而收缩使算子系数 $D/R^2$ 增大 2.8 倍并缩短扩散路径，使干燥加快。
...
表中可见：固定几何下附录 3→附录 4 的物性差异使烘干时长增加 71.93 h，增幅为 $+126.0\%$；收缩在附录 3、附录 4 下分别使时长缩短 31.97 h（$-56.0\%$）与 78.21 h（$-60.6\%$），且 $D$ 越小（干燥越受内部扩散控制）收缩的加速作用越强；两因素叠加（问题 3→问题 4 的实际组合）的净效应为 $-6.3$ h（约 $-11\%$），收缩加速占主导。
...
\item 若 $T_a-2\ ^\circ$C 与 $h_m-50\%$ 两因素的影响近似可叠加，最不利组合下 $t^*$ 增幅估计约 20\%，仍在题述「2--3 天」范围内，模型结论稳健。
\end{enumerate}
