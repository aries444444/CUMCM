...
\textbf{针对问题 2}，采用附录 3 的经验公式，其中密度、比热容、导热系数随水分浓度 $C$ 变化，扩散系数 $D(C,T)=2.4\times10^{-3}\mathrm{e}^{-0.45/C}\mathrm{e}^{-3850/T}$ 随 $C$ 与绝对温度 $T$ 变化，建立热质双向弱耦合模型：温度场通过 Arrhenius 因子影响水分扩散，水分场通过物性参数影响温度场；求解 0--3 h 的温度与水分场（表 3、表 4）。\textbf{针对问题 3}，沿用该模型，烘房条件取附件 1 插值（$t\le14400$ s）与恒温段参数 $(50.165\,^\circ\mathrm{C},\,0.04986\ \mathrm{kg/kg})$ 两段拼接，按判据「各处水分浓度 $\le0.15$ kg/kg」求得烘干时长 $t^*=205701$ s，即 \textbf{57.1392 h}（约 2.38 天），与题述「烘干过程一般持续 2--3 天」相符。

\textbf{针对问题 4}，考虑药材尺寸收缩：采用附录 4 的经验公式，并以附件 2 的半径数据经 Pchip 保形插值构造动边界 $R(t)$；引入随体坐标 $\zeta=r/R(t)$，将热传导与水分扩散方程变换为无对流项的 Lagrange 形式，在同一守恒离散框架下求解。求得含收缩时的烘干时长 $t^*=182897$ s，即 \textbf{50.8049 h}（约 2.12 天）。

\textbf{数值方法与检验}：空间采用严格守恒的有限体积离散（表面节点位于真实表面，Robin 通量与内部界面通量逐时间层对消），时间采用 Crank--Nicolson 格式，三对角方程用 Thomas 算法求解，非线性与双向耦合用交错 Picard 迭代处理。模型通过四重检验：与 Bessel 级数解析解对照，温度场四位小数完全一致；空间步长减半收敛比达理论值 4（问题 1、2 实测比值 4.0），验证二阶精度；离散质量守恒闭合差在 $10^{-7}$ 以内（问题 1 达机器精度）；长时程渐近精确收敛到烘房参数。题给 $D$ 的 Arrhenius 指数对应表观活化能 $E_a=3850R=32.0$ kJ/mol，与文献报道的中药材干燥活化能（30--40 kJ/mol）\cite{danshen,dihuang,baizhu}一致。此外对水分汽化潜热作数量级分析：表面蒸发吸热与对流显热之比约 3--5，其不确定性以时长敏感性区间（约 $+40\%\sim50\%$，见 7.6 节）计入结果解读。
...
药材为长 25 cm、半径 2 cm 的细长圆柱，长径比达 12.5。轴向与径向的扩散特征时间之比约为 $(L/(2R))^2\approx39$（轴向特征尺度取半长 $L/2$，即到最近端面的距离），故端面效应与轴向不均匀性可以忽略，药材内部的温度与水分梯度主要由径向热风对流引起。将药材视为无限长圆柱，取径向一维轴对称模型即可（文献中圆柱形药材、食品的干燥均广泛采用径向一维模型，如人参、豆类、胡萝卜等\cite{simal,abbasi,kim}）。周向按轴对称处理，初始条件均匀，故解不依赖周向角。
...
本题的耦合是\textbf{双向弱耦合}：温度场影响水分场——扩散系数经验式 $D(C,T)$ 含 Arrhenius 因子 $\mathrm{e}^{-3850/T}$，温度升高显著加速水分扩散；水分场影响温度场——附录 3、4 中密度、比热容、导热系数均随水分浓度 $C$ 变化，水分流失改变热容与热阻。两个场通过物性参数相互影响，而边界条件本身不直接交叉（无 Soret/Dufour 交叉效应）。数值上采用交错（staggered）Picard 迭代：同一时间步内先由当前水分场求温度场，再用新温度场求水分场，交错循环直至满足 5.5 节的收敛判据（各算例平均 2.2--3.4 轮，最多 4 轮）。

关于水分汽化的局部吸热效应：严格意义下蒸发吸热会降低表面温度，但本题题给的经验式（$D(C,T)$、$h_m$ 等）是由干燥实验数据拟合得到的\textbf{有效参数}，其数值可能已隐含吸收汽化与吸附等微观机制的净效应——现有信息不足以唯一判定其吸收程度，但若在题给经验式之上再显式加入潜热汇项，将面临与经验参数重复计账的风险，且需引入题给之外的饱和蒸气压等参数。这一判断的数量级依据在 7.6 节给出专门分析（表面蒸发吸热与对流显热之比约 3--5，湿球温度约 41.5 $^\circ$C，对烘干时长的敏感性约 $+40\%\sim50\%$），并在模型假设 H5 中予以声明。
...
\label{eq:zeta}
\end{equation}
各向同性且径向均匀的收缩（假设 H8）意味着任一物质点按统一比例 $R(t)/R(0)$ 收缩，即物质点位置 $r_p(t)=\zeta_pR(t)$、径向速度 $v_r=\zeta_pR'(t)=(R'/R)\,r_p$，故 $\zeta$ 恒标记同一干物质质点。由于干基含水率 $C$ 定义于固相骨架（单位质量干物质的含水质量），对固定 $\zeta$ 的随体导数即骨架物质点上的变化率；在该物质坐标下，随体导数中的对流项与坐标速度项严格相消，扩散算子不含对流项，水分与热量方程化为
\begin{equation}
...
\label{eq:bc4}
\end{equation}
对称条件 $\pdd{T}{\zeta}(0,t)=\pdd{C}{\zeta}(0,t)=0$。表 6 中「到药材中心的距离」列在 $d>R(t)$ 时该位置已在药材之外，表中以 ``---'' 表示（result4.xlsx 中相应单元格按模板留空）；「药材表面」列即 $\zeta=1$ 处的值。固定距离列在 $\zeta$ 坐标的离散节点上经 Pchip 插值取值（插值阶的选择依据见 7.2 节）。
...
以上加密过程均为实测数据：输出行在平滑区收敛比达理论值 4（问题 1、2 实测比值 4.0）、误差按二次律下降，陡梯度区与插值取值处收敛阶偏低（问题 3、4，改用实测阶估计，7.2 节汇总），并与 7.1 节的解析解对照、7.3 节的守恒检验交叉印证。

% ==APPEND==
\newpage
\section{六、求解结果}

以下结果均按题面要求存入 result1--4.xlsx：result1/2.xlsx 含「温度」「水分浓度」两个工作表，A 列为时间（s），第 1 行为到药材中心的距离（cm）；result3/4.xlsx 单表存水分浓度，其末行为烘干结束时刻 $t^*$（205701 s / 182897 s）的一行，与表 5、表 6 的末行一致；所有数值保留四位小数。
...
水分场的变化集中于近表面约 5 mm 的边界层内（$\delta\sim D/h_m\approx5$ mm 为表面浓度剖面的特征长度，即 Robin 边界的外推长度），表面水分浓度由 2.55 kg/kg 降至 1.51 kg/kg，而中心在 1800 s 内基本不变（2.5500 kg/kg）。这一「外层先失水、内部未及响应」的形态正是预热平衡阶段的特征——水分扩散系数仅约 $(4\sim5)\times10^{-9}$ m$^2$/s，水分扩散深度 $\sqrt{Dt}\approx2.6$ mm，远小于半径，故内部含水率尚未显著下降。
...
\begin{equation}
t^*=205701\ \mathrm{s}=\mathbf{57.1392\ h}\ \ (\text{约 2.38 天；空间离散不确定度约}\ \pm0.03\ \text{h，见 7.2 节}),
\end{equation}
...
结果分析：干燥过程呈明显的两阶段形态。前 12 h 含水率快速下降（表面由 2.55 降至 0.164 kg/kg），此为表面传质主导的快速干燥段（干燥速率由表面向环境的对流传质通量控制）；此后干燥速率显著放缓，进入降速干燥段——水分由内部向表面扩散的路径越来越长，且 $D$ 随 $C$ 下降按 $\mathrm{e}^{-0.45/C}$ 急剧衰减，表面含水率 54 h 时仅 0.0528 kg/kg，已逼近烘房水分浓度 0.04986 kg/kg（即趋近平衡含水率）。烘干判据由中心点决定（中心始终是含水率最高处），最终 $t^*=57.14$ h。

\subsection{6.4 问题 4：含收缩的烘干时长（表 6）}

附件 2 的 $R(t)$ 经 Pchip 插值代入 $\zeta$ 坐标离散求解，得
\begin{equation}
t^*=182897\ \mathrm{s}=\mathbf{50.8049\ h}\ \ (\text{约 2.12 天；空间离散不确定度约}\ \pm0.01\ \text{h，见 7.2 节}).
\end{equation}
...
结果分析：半径在干燥前期快速收缩（2 cm$\to$约 1.5 cm 在前 3.5 h），故 1.5 cm、2 cm 两列自 6 h 起即已落在药材之外。与问题 3 对比：$t^*$ 由 57.14 h 缩短为 50.80 h，缩短约 6.3 h（11.1\%）。这一净效应由两个相反方向的因素合成：附录 4 的扩散系数比附录 3 小约 2--5 倍（高含水率端约 5 倍、低含水率端约 2 倍），使干燥变慢；而收缩使算子系数 $D/R^2$ 增大 2.8 倍并缩短扩散路径，使干燥加快。由于问题 3 与问题 4 同时更换了物性与几何，两因素贡献须用同一套离散设置的消融对照分离，结果如表 7 所示。四个组合采用同一套验证离散（$\Delta\zeta=1/160$、物理分辨率 0.125 mm、$\Delta t=5$ s，迭代判据与 5.5 节相同）；附录 3 固定几何值 57.0884 h 与问题 3 生产值（57.14 h，0.0625 mm）相差 0.05 h，属该验证离散的空间偏差，各组合受同向影响，表内百分比结论的影响小于 0.1 个百分点。
...
表中可见：固定几何下附录 3→附录 4 的物性差异使烘干时长增加 71.93 h（$+126.0\%$）；收缩在附录 3、附录 4 下分别使时长缩短 31.97 h（$-56.0\%$）与 78.21 h（$-60.6\%$），且 $D$ 越小（干燥越受内部扩散控制）收缩的加速作用越强；两因素叠加（问题 3→问题 4 的实际组合）的净效应为 $-6.3$ h（约 $-11\%$），收缩加速占主导。
...
\textbf{空间收敛}。问题 1 在 $\Delta r=0.125,0.0625,0.03125$ mm 三种空间步长（$\Delta t=0.125$ s 固定、同一位置采样）下求表 1、表 2：表 1 最大差依次为 $2.30\times10^{-5}$、$5.75\times10^{-6}$ $^\circ$C，比值 4.0；表 2 依次为 $3.51\times10^{-4}$、$8.72\times10^{-5}$ kg/kg，比值 4.0，最大差位于 $(100\ \mathrm{s},$ 表面$)$ 的浓度边界层处。差值比接近理论值 4（二阶格式的理论收敛比，依据 5.5 节的截断误差分析），验证格式为空间二阶收敛；0.03125 mm 步长下表 2 的 Richardson 残余误差约 $2.9\times10^{-5}$，四位小数可信。问题 2 在 0.5/0.25/0.125 mm 三种空间步长下，表 4 最大差依次为 $1.86\times10^{-4}$、$4.66\times10^{-5}$ kg/kg，比值 4.0，同样二阶收敛（表 3 对应差值为 $2.08\times10^{-4}$、$5.21\times10^{-5}\ ^\circ$C）。问题 3 在 0.125/0.0625/0.03125 mm 下的表 5 对齐行逐格差：$0.125\to0.0625$ mm 最大 $1.03\times10^{-4}$、$0.0625\to0.03125$ mm 最大 $4.14\times10^{-5}$ kg/kg，最大差均位于 42 h、1.5 cm 处；表 5 行的实测收敛阶低于格式二阶（末行由 $t^*$ 时间插值取值、最大差位置处于时间-浓度剖面最陡处），按末段实测阶 $p=1.33$ 估计，0.0625 mm 解的残余误差约 $2.7\times10^{-5}$，四位小数可信。问题 4 的 $\zeta$ 方向加密实测：固定距离列经线性插值取值时，$1/40\to1/80\to1/160$ 的表 6 行最大差为 $3.98\times10^{-4}$、$1.58\times10^{-4}$ kg/kg，收敛比仅 2.5——线性插值的 $O(\Delta\zeta^2)$ 误差在 30 h 前后的陡梯度区主导了行差；固定距离列改用 Pchip 三次插值（$C(\zeta)$ 为抛物方程的光滑解，插值误差 $O(\Delta\zeta^3)$）后，$1/80\to1/160\to1/320$ 的最大差降至 $1.48\times10^{-4}$、$5.33\times10^{-5}$ kg/kg，实测收敛阶约 1.5，$1/160$ 离散的 Richardson 估计约 $3.0\times10^{-5}$，四位小数可信。烘干时长 $t^*$ 的空间离散敏感性：问题 3 在 $\Delta r=0.5/0.25/0.125/0.0625/0.03125$ mm 五档下 $t^*$ 为 56.7064、56.9672、57.0874、57.1392、57.1597 h，实测收敛阶 1.12/1.22/1.33，按末段实测阶外推 57.1732 h，与生产值相差 0.034 h，空间离散不确定度约 0.03 h；问题 4 在 $\Delta\zeta=1/20\sim1/320$ 五档下 $t^*$ 为 50.5416、50.6928、50.7713、50.8049、50.8170 h，按实测阶 $p=1.47$ 外推 50.8239 h，不确定度约 0.01 h。
...
按恒等式 \eqref{eq:conserv} 检验全程离散守恒（问题 1 取 1800 s，问题 3、4 取至 $t^*$）：问题 1 在 0.03125 mm 步长下 $\sum v_i\Delta C_i=-5.128889\times10^{-5}$，与 $-R\int h_m(C_R-C_a)\dd t$ 完全相等，闭合差 $3.9\times10^{-19}$；问题 3 闭合差 $6.31\times10^{-11}$（$\sum v_i\Delta C_i=-4.851549\times10^{-4}$；末行由 $t^*$ 时间插值取值，与离散时间层不完全配对，闭合差相对值 $1.3\times10^{-7}$）；问题 4（动边界，按物理体积加权求和、体积收缩引入几何项 $2\dot{R}C/R$）闭合差 $4.1\times10^{-7}$。问题 1 达机器精度、问题 3 达 $10^{-11}$ 量级，验证了「表面通量作用于真实表面节点」离散方案的严格守恒性；问题 4 的残差源于动边界下 $R$ 取半步中点与梯形积分配对的 $O(\Delta t^2)$ 配对误差，相对总失水量仅 $8\times10^{-4}$，可忽略。守恒量按 $\sum v_iC_i$ 计，即水分浓度对体积的积分——它是 $C$-方程本身的离散不变量，本检验针对数值格式的守恒性；物理总水量为 $\int\rho_d C\,\dd V$（$\rho_d=\rho/(1+C)$ 为干物质密度），其中 $\rho_d$ 随 $C$ 的变化已由题给有效扩散系数的标定隐含吸收（假设 H5），故物理水量守恒不能由 $\sum v_iC_i$ 直接检验，本检验的结论限于离散格式层面。
...
\end{equation}
文献报道的中药材热风干燥活化能：丹参 $D_{\mathrm{eff}}=1.7\times10^{-8}\sim9.4\times10^{-8}$ m$^2$/s、$E_a=40.0$ kJ/mol\cite{danshen}；鲜地黄 $E_a=34.94$ kJ/mol\cite{dihuang}；白术 $E_a=37.47$ kJ/mol\cite{baizhu}；题给 $D$ 的数值范围 $10^{-11}\sim10^{-8}$ m$^2$/s 亦与上述实测同量级。国内对圆柱形药材热风干燥的传热传质数值模拟亦有报道\cite{wang,pang}。$E_a=32.0$ kJ/mol 高于纯液态水自扩散活化能（18--20 kJ/mol\cite{mills}）而低于化学吸附能级，处于解吸-汽化控制的合理区间，说明题给经验式在物理上自洽。此外 $t^*=57.14$ h、50.80 h（约 2.1--2.4 天）均落在题述「2--3 天」范围内，进一步佐证模型参数与结构的合理性。

\subsection{7.6 汽化潜热影响的数量级分析}

水分汽化在表面吸热，可能使表面温度低于环境温度。按题设的有效模型观点（假设 H5），潜热效应已被经验参数吸收；为定量说明其影响程度，作如下数量级估计。

\textbf{(1) 湿球温度估计。}恒温段空气状态为 $T_a=50.165\ ^\circ$C、$C_a=0.04986$ kg/kg。附件 1 未指明 $C_a$ 是空气含湿比还是药材的等效平衡水分浓度；本节按含湿比解释（与 Robin 边界中直接相减的单位一致）：50 $^\circ$C 下空气的饱和含湿量约 0.0868 kg/kg\cite{incropera}，故相对湿度 $\mathrm{RH}\approx57\%$。由焓湿图（或湿球温度方程）求得湿球温度 $T_{wb}\approx41.5\ ^\circ$C，即表面若完全由蒸发吸热平衡，其温度下限约为 41.5 $^\circ$C。若 $C_a$ 实为等效平衡水分浓度，则它直接给出表面趋近的平衡态，本节关于潜热影响的数量级结论不受解释差异影响。
...
为评估模型结论对输入参数的稳健性，对问题 3、4 分别取 8 个参数变体：烘房温度 $T_a\pm2\ ^\circ$C、烘房水分浓度 $C_a\pm20\%$、对流换热系数 $h\pm50\%$、对流传质系数 $h_m\pm50\%$，其余参数保持题给值，重算烘干时长，结果如表 8 所示（灵敏度为两个变体烘干时长之差，对空间离散不敏感，故在较粗的验证空间离散上计算：问题 3 取 0.25 mm、问题 4 取 $\Delta\zeta=1/80$）。
...
\item 数值格式严格守恒：表面 Robin 通量直接作用于真实表面节点，与内部界面通量逐层对消，离散质量守恒闭合差在 $10^{-7}$ 以内（问题 1 达机器精度 $10^{-18}$）。
