...
圆柱轴对称热传导 + 水分扩散（非线性 D(C)），守恒型有限体积离散 + Crank--Nicolson。
附件读取、三对角求解、CN 单步等公共数值内核统一取自 common.py。
...
from common import ROOT, crank_radial, load_annex, setup_console
...
def crank_step(u, K, beta, bc_prev, bc_next, dt):
    """守恒型有限体积 + Crank--Nicolson 一步（此处 Cp≡1，K 吸收热扩散率；见 common.crank_radial）。
    热: K=alpha, beta=h/k；质: K=D(C), beta=h_m/D_s。"""
    return crank_radial(u, K, np.ones(N), beta, bc_prev, bc_next, dt, N, dr)
